...
for i in range(int(n)):
    ...
    if game_player not in game_player_list : # 게임을 플레이한 사람의 명단에 없다면
        game_player_list[game_player] = 0 # 게임을 플레이한 사람의 이름을 저장하는 dict 에 저장
        game.append(game_player) # 현재 게임 플레이어로 추가
    if games[gameMode]-1 == len(game): # 현재 게임을 플레이할 사람의 인원과 게임을 진행해야 할 인원에서 임스를 뺀 값이 같다면
